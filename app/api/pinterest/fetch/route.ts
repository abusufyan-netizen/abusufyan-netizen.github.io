...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Extract JSON data from script tags (similar to PinFlow PHP logic)
    const patterns = [
    ...
    if (!rawData) {
      // Fallback: try to find any large JSON object in scripts
      ...
      if (fallbackMatch) {
        for (const script of fallbackMatch) {
          if (script.includes('images') && script.includes('736x')) {
            // Very basic heuristic for Pinterest JSON
            try {
